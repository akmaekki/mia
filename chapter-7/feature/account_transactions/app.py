...
class AccountTransactionsService:
    ...
    statsd = StatsClient('statsd-agent', 8125, prefix='simplebank-demo.account-transactions')

    @rpc
    @statsd.timer('request_reservation')
    def request_reservation(self, payload):
        print("[{}] {} received request to reserve stocks... reserving".format( payload, self.name))
